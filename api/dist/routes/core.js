...
export async function registerCoreRoutes(app) {
    ...
    app.post("/backup", async (request) => {
        const result = await pool.query(`insert into backup_records (name, status, size_bytes, created_by, completed_at)
       values ($1, 'ready', 0, $2, now())
       returning id, name, created_at as "createdAt", size_bytes as "sizeBytes", status`, [`Manual backup ${new Date().toISOString()}`, actorId(request)]);
