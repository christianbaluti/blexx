...
async function upsertGeneratedNotifications(rows: GeneratedNotification[]) {
  if (!rows.length) return;
  await Promise.all(rows.map((row) => pool.query(
    `insert into notifications (source_key, type, severity, title, body, entity, entity_id, status)
    ...
    [row.sourceKey, row.type, row.severity, row.title, row.body, row.entity ?? null, row.entityId ?? null, row.status ?? "pending"]
  )));
}
...
export async function registerCoreRoutes(app: FastifyInstance) {
  ...
  app.get("/notifications", async (request) => {
    ...
    const generatedNotifications: GeneratedNotification[] = [
      ...lowStock.rows.map((row) => ({
        ...
        severity: (numberify(row.quantity) <= 0 ? "critical" : "warning") as GeneratedNotification["severity"],
  ...
  app.delete("/items/:id", async (request) => {
    ...
    if (linked.rows.length) {
      ...
      if (!archived.rowCount) throw app.httpErrors.notFound("Item not found");
      return { ok: true, archived: true };
    }
    const deleted = await pool.query("delete from items where id = $1 returning id", [id]);
    if (!deleted.rowCount) throw app.httpErrors.notFound("Item not found");
  ...
  app.delete("/products/:id", async (request) => {
    ...
    if (linked.rows.length) {
      ...
      if (!archived.rowCount) throw app.httpErrors.notFound("Product not found");
      return { ok: true, archived: true };
    }
    const deleted = await pool.query("delete from products where id = $1 returning id", [id]);
    if (!deleted.rowCount) throw app.httpErrors.notFound("Product not found");
    return { ok: true, deleted: true };
  });
  ...
  app.post("/inventory/adjustments", async (request) => {
    ...
    try {
      ...
      let locationType: "warehouse" | "shop" = body.outletId === "shop" ? "shop" : "warehouse";
      if (body.outletId === "warehouse" || body.outletId === "shop") {
        ...
        locationId = location.rows[0].id as string;
        locationType = location.rows[0].type as "warehouse" | "shop";
      }
      ...
      if (body.itemId) {
        if (locationType !== "warehouse") throw app.httpErrors.badRequest("Raw items can only be adjusted in the warehouse.");
        if (direction === "out") await assertWarehouseItem(client, body.itemId, qty);
        ...
        if (locationType === "shop") {
          if (direction === "out") await assertShopProduct(client, body.productId, qty);
          await updateShopProduct(client, body.productId, direction === "in" ? qty : -qty);
        } else {
          if (direction === "out") await assertWarehouseProduct(client, body.productId, qty);
    ...
    try {
      ...
      for (const line of body.items) {
        ...
        await client.query(
          `insert into sale_items (sale_id, product_id, quantity, unit_price, discount, unit_cost, line_total)
           values ($1,$2,$3,$4,$5,$6,$7)`,
          [sale.rows[0].id, line.productId, line.quantity, line.unitPrice, line.discount, unitCost, lineTotal]
        );
        await updateShopProduct(client, line.productId, -line.quantity);
        await client.query(
          `insert into stock_movements (location_id, product_id, direction, quantity, unit_cost, ref_type, ref_id, user_id, note)
  ...
  app.post("/users", async (request, reply) => {
    ...
    if (config.smtpHost && config.smtpUser && config.smtpPass) {
      try {
        ...
        const settings = settingsResult.rows[0]?.value as { downloads?: { androidUrl?: string; iosUrl?: string }; company?: { tradingName?: string } } | undefined;
        await mailTransport().sendMail({
          from: config.smtpFrom,
          to: body.email,
